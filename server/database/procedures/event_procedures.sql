...
BEGIN
  ...
  SET contact_id_in = (SELECT contact_id
                       FROM contact
                              LEFT JOIN user u on contact.contact_id = u.contact
                       WHERE u.user_id = event_organizer_in
                       LIMIT 1);
  INSERT INTO event (event_id, title, description, location, start_time, end_time, category, capacity, organizer,
                     cancelled)
